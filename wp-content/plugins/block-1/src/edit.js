...
export default function Edit(props) {
	...
	const gmfgb_update_repeater_item = (
		image,
		image_caption,
		selectedVideoType,
		video_media,
	...
	const imageSizes = select("core/editor")
		.getEditorSettings()
		.imageSizes.map((size) => size.slug);
	...
	return (
		<>
				...
				<Panel>
					<PanelBody title="MediaGrid Settings">
						...
						<ToggleControl
							...
							onChange={(newIsfancyBoxEnabled) =>
								setAttributes({ fancyBoxEnabled: newIsfancyBoxEnabled })
							}
							...
							<>
								<fieldset>
									<ToggleControl
										...
										onChange={(newVideoOptionEnabled) =>
											setAttributes({
								...
								<fieldset>
									<legend className="blocks-base-control__label">
										{__(
											"Fancybox Video Options, Please hover Media Image to upload or add Video",
											"grid-masonry-for-guten-blocks",
										)}
									</legend>
								</fieldset>
						...
						<SelectControl
							...
							onChange={(newSelectedSize) =>
								setAttributes({ selectedSize: newSelectedSize })
							}
						/>
						...
						<>
							...
							<RangeControl
								value={gridItem}
								onChange={(newGridItem) =>
									setAttributes({ gridItem: newGridItem })
								}
								min={1}
								max={5}
							/>
			...
			>
				{attributes.items.map((item, index) => (
					<div className="gmfgb-mg-wrap" key={index}>
						...
						<MediaUploadCheck>
							<MediaUpload
								onSelect={(image) =>
									gmfgb_update_repeater_item(
										image,
										item.image_caption,
										item.selectedVideoType,
										item.video_media,
										item.popup_url,
										index,
									)
				...
				>
					<svg width="50" viewBox="0 0 32 32" fill="#000000">
						<g id="SVGRepo_bgCarrier" stroke-width="0"></g>
						<g
							id="SVGRepo_tracerCarrier"
							stroke-linecap="round"
